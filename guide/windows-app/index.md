...
パソコン（Windows）から、オンラインデイサービス「長老大学」に参加するためのアプリの使い方です。
**毎朝ブラウザを開いてURLを入力する手間がなくなり**、時間になるとアプリが自動で待機室に入ります。

> 📌 **このアプリは「長老大学」をご契約中の方向け**です。参加には、いつものログイン（メールアドレス・パスワード）が必要です。
...
アプリを初めて起動すると、設定画面が開きます。**いつものログイン情報**を入れて保存します。

![設定画面](images/03-settings.png)

1. **① ログイン情報** に、**メールアドレス**と**パスワード**（noiz.funのログイン）を入力
2. **「保存」** を押す
